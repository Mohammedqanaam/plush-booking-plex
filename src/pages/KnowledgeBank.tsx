...
const KnowledgeBank = () => {
  ...
  const [selected, setSelected] = useState<{ title: string; summary: string; details: string; tags: string[]; branch?: string; brand?: string; attachmentUrl?: string } | null>(null);
    ...
    <div className="p-4 max-w-7xl mx-auto space-y-4">
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelected(null)}>
        <DialogContent className="max-w-2xl">
          {selected ? <><DialogHeader><DialogTitle>{selected.title}</DialogTitle></DialogHeader><p className="text-sm text-muted-foreground">{selected.brand ?? "Global"} · {selected.branch ?? "General"}</p><p className="whitespace-pre-line text-sm">{selected.details}</p>{selected.attachmentUrl ? <a className="inline-flex h-9 items-center rounded-lg border px-3 text-xs hover:border-primary/60" href={selected.attachmentUrl} target="_blank" rel="noreferrer">فتح المرفق PDF</a> : null}<div className="flex flex-wrap gap-2">{selected.tags.map((tag) => <span key={tag} className="text-xs px-2 py-1 rounded bg-secondary border">{tag}</span>)}</div></> : null}
        </DialogContent>
      </Dialog>
    </div>
  );
