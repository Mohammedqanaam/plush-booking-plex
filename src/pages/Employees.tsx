...
const Employees = () => {
  ...
  return <div className="p-4 max-w-5xl mx-auto space-y-4">
    <div className="glass-card p-4 space-y-3">
      ...
      {!canManage && <p className="text-xs text-muted-foreground">وضع المشاهدة فقط: إدارة الأسماء والإخفاء متاحة للمستخدمين المصرح لهم فقط.</p>}
      ...
      <div className="space-y-2">
        {shown.map((employee) => (
          <div key={employee.agent} className="border rounded-lg p-3 space-y-2">
            {canManage ? (
              <input
                className="h-9 rounded border px-2 w-full bg-secondary"
                value={employee.display}
                onChange={(e) => setRenames((p) => ({ ...p, [employee.agent]: e.target.value }))}
              />
            ) : (
              <p className="font-medium text-sm">{employee.display}</p>
            )}
